...
## Skills — 한글 수식·풀이 규칙 (2026-05-03)

두 SOT 정본 + 두 skill 짝으로 운영. Claude가 작업 종류에 따라 자동 호출.

| 정본 | skill | 호출 시점 |
|---|---|---|
| `한글 수식입력기_260425.txt` | `hmr` | 한글(.hwp/.hwpx) 파일 생성, 수식 코드 `[ ]` 작성/검토, 프롬프트 수정 시 |
| `docs/고등수학_풀이_규칙.md` | `hsr` | 풀이/해설 한국어 서술·검수, 학년별 단원 누적 시 |

- 한글 파일 출력에는 둘 다 호출 / 수식 코드만 다룰 땐 hmr만 / 해설 검수 땐 hsr만
- 새 결함·새 단원 발견 시 **SOT 본체 먼저 갱신** → 코드/프롬프트는 따라감 (역방향 금지)

## 그래프 자동 검증

hwpx 그래프 임베딩 작업 후 즉시 검증:
```bash
python backend/scripts/validate_graph_hwpx.py 파일.hwpx
```
SOT: `docs/그래프_렌더_규칙.md` 규칙 9 자동 검증 체크리스트의 실행판.
